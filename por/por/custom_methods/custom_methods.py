...
def calculate_rate_and_uom(doc, method):
    ...
    for item in doc.items:
        if item.date_out and item.date_returned:  # Simplified attribute check
            ...
            four_hour_rate = frappe.get_value("Item Price", {"item_code": item.item_code, "uom": "4hr"}, "price_list_rate")
            ...
            # check if date/time returned is earlier than date out!
            if duration <= 0:
                frappe.throw('Please verify date and time returned is after date and time out.')

            # calculate for half day rate when returned on time
            elif duration <= (4 + settings.four_hr_grace):
                rate = four_hour_rate
                uom = '4hr'
                rental_units = '4hr:1'
...
def get_items():
    ...
    for item in itemsWithCount:  # traverse through items
        item['rent_count'] = frappe.db.count('Sales Invoice Item', {'item_code' : item.item_code}) # add a value to store rent count
